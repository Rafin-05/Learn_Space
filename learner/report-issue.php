...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$subject || !$message) {
        ...
        $stmt->bind_param("sissss", $type, $user_id, $name, $email, $subject, $message);
        $stmt->execute();
        $success = "Your message has been sent to the admin. We'll get back to you soon!";
    }
}

// Previous messages
$my_messages_q = $conn->query("SELECT * FROM support_messages WHERE sender_type='learner' AND sender_id=$user_id ORDER BY created_at DESC");
$my_messages = [];
while ($r = $my_messages_q->fetch_assoc()) $my_messages[] = $r;

// Mark resolved messages as seen (closed) now that user has opened this page
$conn->query("UPDATE support_messages SET status='closed' WHERE sender_type='learner' AND sender_id=$user_id AND status='resolved'");
$has_resolved = false;
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Report an Issue - LearnSpace</title>
<link rel="stylesheet" href="../assets/css/style.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
<style>
.report-form { background:white;border-radius:var(--radius);padding:28px;border:1px solid var(--gray-border);max-width:680px; }
.form-group { margin-bottom:16px; }
.form-group label { display:block;font-weight:700;font-size:.88rem;margin-bottom:6px;color:var(--black); }
.form-group input,.form-group textarea,.form-group select { width:100%;padding:10px 14px;border:1px solid var(--gray-border);border-radius:var(--radius-sm);font-size:.9rem;font-family:inherit;resize:vertical }
...
<div class="dashboard-layout">
  <aside class="sidebar">
    ...
    <ul class="sidebar-nav">
      ...
      <li><a href="report-issue.php" class="active"><i class="fas fa-flag"></i> Report an Issue<?php if($has_resolved): ?> <span title="One or more issues resolved" style="display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;background:#16a34a;color:white;border-radius:50%;font-size:.7rem;font-weight:900;margin-left:4px">!</span><?php endif; ?></a></li>
          ...
          <div class="report-form">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;padding:14px;background:var(--primary-bg);border-radius:var(--radius-sm)">
              ...
              <div>
                ...
                <div style="font-size:.8rem;color:var(--gray)"><?= htmlspecialchars($user['email']) ?></div>
              </div>
            </div>
            <form method="POST">
              <div class="form-group">
                <label><i class="fas fa-tag"></i> Subject *</label>
                <input type="text" name="subject" placeholder="Brief summary of your issue..." required value="<?= htmlspecialchars($_POST['subject'] ?? '') ?>">
              </div>
              <div class="form-group">
                <label><i class="fas fa-comment-alt"></i> Message *</label>
                <textarea name="message" rows="6" placeholder="Describe your issue in detail. Include any relevant course or lesson names..." required><?= htmlspecialchars($_POST['message'] ?? '') ?></textarea>
              </div>
