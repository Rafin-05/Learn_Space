        ...
        if ($user && password_verify($password, $user['password'])) {
            if ($user['status'] === 'blocked') {
                ...
                if ($last_login !== $today) {
                    ...
                    $update_streak = $conn->prepare("UPDATE users SET last_login_date = ?, current_streak = ? WHERE id = ?");
                    $update_streak->bind_param("sii", $today, $streak, $user['id']);
...
<title>Sign In - LearnSpace</title>
...
<div class="auth-page">
  <div class="auth-card">
    ...
    <div class="role-tabs">
      <a href="login.php" class="role-tab active">🎓 Learner</a>
      <a href="../instructor/login.php" class="role-tab">👨‍🏫 Instructor</a>
    </div>
    ...
    <div class="auth-footer">
      New to LearnSpace? <a href="register.php">Create free account</a>
    </div>
  </div>
</div>
