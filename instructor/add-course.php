    ...
    if (empty($title) || empty($description)) {
        ...
        foreach ($units as $u_idx => $unit) {
            ...
            foreach ($lessons as $l_idx => $lesson) {
                ...
                if ($l_type === 'coding') {
                    $c_lang = trim($lesson['language'] ?? '');
                    $c_boil = trim($lesson['code'] ?? '');
                    $c_exp = trim($lesson['expected'] ?? '');
                    
                    if(empty($c_lang) || empty($c_exp)) continue;
                    
                    $l_stmt = $conn->prepare("INSERT INTO lessons (unit_id, title, lesson_type, lesson_link, coding_language, coding_boilerplate, coding_expected_output, lesson_order) VALUES (?, ?, 'coding', NULL, ?, ?, ?, ?)");
                    $l_stmt->bind_param("issssi", $unit_id, $l_title, $c_lang, $c_boil, $c_exp, $l_idx);
                    $l_stmt->execute();
                } else {
                    $l_link = trim($lesson['link'] ?? '');
                    if (empty($l_link)) continue;
                    
                    $l_stmt = $conn->prepare("INSERT INTO lessons (unit_id, title, lesson_type, lesson_link, lesson_order) VALUES (?, ?, 'video', ?, ?)");
...
function addUnit() {
  ...
  div.innerHTML = `
    ...
    <div style="display:flex; gap:10px;">
      ...
      <button type="button" class="add-lesson-btn" style="border-color:#10b981; color:#10b981;" onclick="addCodingChallenge(${idx})">
        <i class="fas fa-code"></i> Add Coding Challenge
      </button>
    </div>
  `;
...
function addLesson(unitIdx) {
  ...
  div.innerHTML = `
    ...
    <input type="hidden" name="units[${unitIdx}][lessons][${lIdx}][type]" value="video">
...
function addCodingChallenge(unitIdx) {
  if (!lessonCounts[unitIdx]) lessonCounts[unitIdx] = 0;
  lessonCounts[unitIdx]++;
  const lIdx = lessonCounts[unitIdx];
  const container = document.getElementById(`lessons_${unitIdx}`);
  ...
  div.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      <strong style="font-size:0.88rem;color:#10b981"><i class="fas fa-code"></i> Coding Challenge ${lIdx}</strong>
      <button type="button" onclick="removeLesson(${unitIdx},${lIdx})" style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:0.85rem"><i class="fas fa-times"></i> Remove</button>
    </div>
    <input type="hidden" name="units[${unitIdx}][lessons][${lIdx}][type]" value="coding">
    <div class="form-group">
      <label style="font-size:0.83rem">Challenge Title *</label>
      <input type="text" name="units[${unitIdx}][lessons][${lIdx}][title]" class="form-control" placeholder="e.g. Write a Hello World Function" required>
    </div>
    <div class="form-group">
      <label style="font-size:0.83rem">Language *</label>
      <select name="units[${unitIdx}][lessons][${lIdx}][language]" class="form-control" required>
        <option value="63">JavaScript (Node.js 12)</option>
        <option value="71">Python (3.8)</option>
        <option value="68">PHP (7.4)</option>
        <option value="54">C++ (GCC 9)</option>
        <option value="62">Java (OpenJDK 13)</option>
      </select>
    </div>
    <div class="form-group">
      <label style="font-size:0.83rem">Boilerplate Code (Optional)</label>
      <textarea name="units[${unitIdx}][lessons][${lIdx}][code]" class="form-control" rows="4" style="font-family:monospace" placeholder="Initial code given to student..."></textarea>
    </div>
    <div class="form-group">
      <label style="font-size:0.83rem">Expected Output (Exact String) *</label>
      <textarea name="units[${unitIdx}][lessons][${lIdx}][expected]" class="form-control" rows="3" style="font-family:monospace" placeholder="e.g. Hello World" required></textarea>
    </div>
  `;
  container.appendChild(div);
}
